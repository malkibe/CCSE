...
ini_set('display_errors', 0);
    ...
    session_start();
    $stmt = $mysqli->prepare("SELECT * FROM `report`  WHERE `categoryname` = ?");
    $stmt->bind_param("s", $_SESSION['tech_categ']);
	$stmt->execute();
	$result = $stmt->get_result();
	if($result->num_rows > 0){
         ;
     } else {
            $message = "You don\'t have any report issues , Thank You";
                                ...
                                <div class="form-horizontal">
                                    ...
                                    <?php
                                            ...
                                            $total_pages = ceil($total_rows / $no_of_records_per_page);
                                                ...
                                                $fixed = "Fixed";
                                                $stmt = $mysqli->prepare("SELECT * FROM `report`  WHERE `categoryname` = ? AND `status` <> ? LIMIT $offset , $no_of_records_per_page");
                                                $stmt->bind_param("ss", $_SESSION['tech_categ'],$fixed);
                                                $stmt->execute();
                                                $result = $stmt->get_result();
                                                if($result->num_rows > 0){
                                                while($row = $result->fetch_assoc()) {
                                                   ...
                                                   echo "<form action='report_issues_update.php' method='post'>";
                                                   echo "<input type='hidden' name='id' value='".$id."'>";
                                                   echo "<input type='submit' name='submit' class='btn btn-info' value='View'>";
                                                   echo "</form>";
                                                   echo "</div>";
                                                   echo "</div>";
                                            }
